...
# possible actions are right, stay,shoot
...
def west(p, m, a, ms, mh, u, stepcost, gamma):
    ...
    t2 = (gamma * ((p2 * u[0][m][a][ms1][mh]) + (p1 * u[0][m][a][ms][mh])))
    if(ans < t2):
        ans = t2
        action = "STAY"
...
def north(p, m, a, ms, mh, u, stepcost, gamma):
    ...
    t2 = (gamma * ((p1 * ((0.85*u[2][m][a][ms][mh]) + (0.15*u[4][m][a][ms][mh]))) + (
...
def south(p, m, a, ms, mh, u, stepcost, gamma):
    ...
    ans = t1
    action = "UP"
    t2 = (gamma * ((p1 * ((0.85*u[3][m][a][ms][mh]) + (0.15*u[4][m][a][ms][mh]))) + (
